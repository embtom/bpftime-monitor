...
FdSet::FdSet(FdSet&& other) noexcept :
 unBlockFd_(std::exchange(other.unBlockFd_, -1)), entries_(std::move(other.entries_))
{
}
...
FdSetRet FdSet::Select(std::chrono::milliseconds timeout) const
{
    return Select(nullptr, timeout);
}
...
FdSetRet FdSet::Select() const
{
    return Select(nullptr, std::chrono::milliseconds::max());
}
